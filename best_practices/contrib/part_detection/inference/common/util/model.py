...
def forward_nms_script_mycoco(model, dataloader, cfg):
    pred_results = []
    for (img, targets, paths, shapes) in tqdm(dataloader):
        ...
        # non_max_suppression
        # box_out = non_max_suppression(box_out, conf_thres=cfg["conf_thres"], iou_thres=cfg["iou_thres"])
        boxout = nms(box_out, conf_thres=cfg["conf_thres"], iou_thres=cfg["iou_thres"])
        for idx, pred in enumerate(boxout):
            try:
                scale_coords(img[idx].shape[1:], pred[:, :4], shapes[idx][0], shapes[idx][1])  # native-space pred
        ...
        #     if a["score"] > 0.9:
        #         annotator.box_label(a["bbox"], "err", color=colors(0, True))
        # im0 = annotator.result()
        # cv2.imwrite("a.jpg", im0)

    return pred_results,boxout,im0,img
