...
def evaluate(cocoGt_file, cocoDt_file):
    cocoGt = COCO(cocoGt_file)                      # 导入验证集，标注文件的路径及文件名，json文件形式
    cocoDt = cocoGt.loadRes(cocoDt_file)            # 导入验证集的标签信息
...
## evaluate_customised_dataset


def evaluate_customised_dataset(cocoGt_file, cocoDt_file):
    # 根据数据集特征，进行校验
    # 加载ground truth annotations和detection结果
    # cocoGt = 
