...
def main(): # pylint: disable=R0914
    ...
    input_tensor = graph.get_tensor_by_name('input_data:0')
    output_tensor = graph.get_tensor_by_name('resnet_model/final_dense:0')
    ...
    amct.save_model(model_file, ['resnet_model/final_dense'], record_file, os.path.join(OUTPUTS, 'resnet-50_v1'))
    ...
    with tf.Session(graph=graph_reload) as session:
        fakequant_prediction = session.run('resnet_model/final_dense:0', feed_dict={'input_data:0': image_test})
