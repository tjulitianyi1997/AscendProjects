...
RESNET50_CAFFEMODEL_URL='https://obs-9be7.obs.cn-east-2.myhuaweicloud.com/003_Atc_Models/'\
    'AE/ATC%20Model/resnet_50/ResNet-50-model.caffemodel'
...
def download_caffemodel():
    caffemodel_path = os.path.join(ROOT_DIR, 'model',
                                   'ResNet-50-model.caffemodel')
    if os.path.exists(caffemodel_path):
        print("[INFO]'{}' already exist, no need to download.".format(
              caffemodel_path))
        return
    urllib.request.urlretrieve(RESNET50_CAFFEMODEL_URL, caffemodel_path)
    print(
        "[INFO]Download 'ResNet-50-model.caffemodel' to '{}' success.".format(
            caffemodel_path))
