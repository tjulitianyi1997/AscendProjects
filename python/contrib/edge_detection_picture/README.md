...
2. Obtain the source network model required by the application.
    ...
    | ------------ | --------------------- | ------------------------------------------------------------ |
    | RCF          | Edge detection based on Caffe.| Download the model and weight files by referring to links in **README.md** in the [ATC_RCF_Caffe_AE](https://gitee.com/ascend/ModelZoo-TensorFlow/tree/master/TensorFlow/contrib/cv/edge_detection/ATC_RCF_Caffe_AE) directory of the ModelZoo repository.|
    ```
