...
void Call::server_callback_(rclcpp::Client<wheeltec_mic_msg::srv::GetOfflineResultsrv>::SharedFuture result)
{
	...
	if (str3 == response->text){	
		...
		call_ = true;
		recognize_fail_count = 0;
		//cout << "已识别到指令:[ " << response->text <<"]"<< endl;
	}
...
int Call::request_()
{
	...
	auto request = std::make_shared<wheeltec_mic_msg::srv::GetOfflineResultsrv::Request>();
	/***离线命令词识别服务参数设置***/
	...
	// cout<<" time_per_order  = "<< seconds_per_order <<endl;
	/***循环频率Hz***/
...
: rclcpp::Node(node_name,options){
	...
	/***离线命令词识别服务客服端创建***/
	Call_Client_ = this->create_client<wheeltec_mic_msg::srv::GetOfflineResultsrv>(
		"get_offline_recognise_result_srv");
}
