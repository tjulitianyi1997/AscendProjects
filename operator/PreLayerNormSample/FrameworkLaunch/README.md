...
| 目录                  | 描述                   |
|---------------------|----------------------|
| PreLayerNormCustom.json       | PreLayerNormCustom算子的原型定义json文件 |
| [PreLayerNormCustom](./PreLayerNormCustom)       | PreLayerNormCustom算子工程 |
| [AclNNInvocation](./AclNNInvocation) | 通过aclnn调用的方式调用PreLayerNormCustom算子工程。 |
...
### 2.编译算子工程
  ...
  - 执行如下命令，切换到算子工程PreLayerNormCustom目录
    ...
    ```
    cd $HOME/samples/operator/PreLayerNormCustomSample/FrameworkLaunch/PreLayerNormCustom
    ```
...
## 通过aclnn调用的方式调用PreLayerNormCustom算子工程
...
### 样例运行
  ...
  - 进入到样例目录
    ...
    ```    
    cd $HOME/samples/operator/PreLayerNormCustomSample/FrameworkLaunch/AclNNInvocation
    ```
...
## 更新说明
  | 时间 | 更新事项 |
|----|------|
| 2023/11/21 | 新增AclNNInvocation样例 |
