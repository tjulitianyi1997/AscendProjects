...
| [LayerNormCustom](./LayerNormCustom)       | LayerNormCustom算子工程 |
| [AclNNInvocation](./AclNNInvocation) | 通过aclnn调用的方式调用LayerNormCustom算子工程。 |
...
### 2.编译算子工程
  ...
  - 执行如下命令，切换到算子工程LayerNormCustom目录
    ...
    ```
    cd $HOME/samples/operator/LayerNormCustomSample/FrameworkLaunch/LayerNormCustom
    ```
...
## 通过aclnn调用的方式调用LayerNormCustom算子工程
...
### 样例运行
  ...
  - 进入到样例目录
    ...
    ```    
    cd $HOME/samples/operator/LayerNormCustomSample/FrameworkLaunch/AclNNInvocation
    ```
