...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnMatmulLeakyreluCustomGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2], outputTensor_[0],
    ...
    ret = aclnnMatmulLeakyreluCustom(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
