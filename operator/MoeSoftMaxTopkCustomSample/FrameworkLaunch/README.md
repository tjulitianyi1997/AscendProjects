...
| [MoeSoftMaxTopkCustom](./MoeSoftMaxTopkCustom)       | MoeSoftMaxTopkCustom算子工程 |
| [AclNNInvocation](./AclNNInvocation) | 通过aclnn调用的方式调用MoeSoftMaxTopkCustom算子工程。 |
...
### 2.编译算子工程
  ...
  - 执行如下命令，切换到算子工程MoeSoftMaxTopkCustom目录
    ...
    ```
    cd $HOME/samples/operator/MoeSoftMaxTopkCustomSample/FrameworkLaunch/MoeSoftMaxTopkCustom
    ```
...
## 通过aclnn调用的方式调用MoeSoftMaxTopkCustom算子工程
...
### 样例运行
  ...
  - 进入到样例目录
    ...
    ```    
    cd $HOME/samples/operator/MoeSoftMaxTopkCustomSample/FrameworkLaunch/AclNNInvocation
    ```
