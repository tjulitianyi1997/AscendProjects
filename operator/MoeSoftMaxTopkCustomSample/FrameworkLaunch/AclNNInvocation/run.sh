...
if [ -n "$CASEX" ]; then  
  echo "run case is $CASEX"  
else  
  CASEX="case1"
  echo "run default case $CASEX"  
fi
...
function main {
    ...
    ./execute_moe_soft_max_topk_op $CASEX
    ...
    # 5. 比较真值文件
    cd $CURRENT_DIR
    ret=`python3 scripts/verify_result.py output/output_y.bin output/output_indices.bin output/golden_y.bin output/golden_indices.bin`
    echo $ret
